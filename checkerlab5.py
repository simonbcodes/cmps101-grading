...
requiredFiles = ["test-input.txt", "test-output.txt"]
...
def grade(flag):
	if "-s" in flag:
		if len(sys.argv) > 2:
			paths = lookForDirectory("lab5/" + sys.argv[2], "lab5")
			print("lab5/" + sys.argv[2])
			print(paths[0])
			gradeStudent(paths[0])
		else:
			print("Usage: checkerlab5.py -s lab5/studentDirectory")
...
def gradeStudent(path):
	global wrong
	os.chdir(path)
	if fileExists("Makefile") and fileExists("README") and fileExists("queue.h") and fileExists("queue.c") and fileExists("queueClient.c"):
		os.chdir(root)
		copyFiles(path)
		...
		call(["make"])
		print("---")
		if fileExists("queueClient"):
			print("---")
			checkCFile("test-input.txt", "test-output.txt", "queueClient")
			print("---")
